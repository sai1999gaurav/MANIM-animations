...
class Scene3(GraphScene):
    ...
    def construct(self):
        ...
        img_name = './images/kai_window' + str(w_length) + '.jpg'
        ...
        list2 = TexMobject(r"\text{Kaiser window}",r"\text{ N = }", str(w_length))
    ...
    def rect_dtft_2(self,N):
        Xi = [np.i0(beta*N*np.sqrt(1-(i*i)/(N*N)))/np.i0(beta*N) for i in range(-N,N+1)]
        X = np.fft.fft(Xi, dtft_size)
        X = np.fft.fftshift(X) - 0.35
