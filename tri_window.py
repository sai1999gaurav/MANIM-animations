...
class Scene3(GraphScene):
    ...
    def construct(self):
        ...
        img_name = './images/tri_window' + str(w_length) + '.jpg'
        ...
        list2 = TexMobject(r"\text{triangular window}",r"\text{ N = }", str(w_length))
        list3 = TexMobject(r"\text{with DTFT:}")        
        list2.set_color_by_tex_to_color_map({
            "triangular window": BLUE
        })
        ...
        w_max = w_length + 4*8
        w_min = w_length + 8
        for w_length in range(w_min,(w_max + 1),8):
            #self.remove(c_graph)
        ...
        list2 = TexMobject(r"\text{are almost negligible}")
        list3 = TexMobject(r"\text{as the relative side-lobe}")
