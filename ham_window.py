...
class Scene3(GraphScene):
    ...
    def construct(self):
        ...
        img_name = './images/ham_window' + str(w_length) + '.jpg'
        ...
        list2 = TexMobject(r"\text{Hamming window}",r"\text{ N = }", str(w_length))
        ...
        sentence.scale(0.7)
        self.play(Write(sentence))
        list1 = TexMobject(r"\text{Band Tolerances are high}")
        list2 = TexMobject(r"\text{enough owing to significant}")
    ...
    def rect_dtft_2(self,N):
        Xi = [(0.54 + 0.46*np.cos(np.pi*i/N)) for i in range(-N,N+1)]
