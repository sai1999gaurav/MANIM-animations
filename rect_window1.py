...
class Scene3(GraphScene):
    ...
    def construct(self):
        ...
        img.shift((t_i_up-0.5)*UP + 1.4*t_i_side*RIGHT)  # Move the image
        ...
        w_max = 39
        for w_length in range(5,(w_max + 1),8):
            #self.remove(c_graph)
            ...
            self.play(Write(sentence))
            img_name = './images/rect_window' + '5' + '.jpg'
            img = ImageMobject(img_name)
            img.scale(1.5)  # Resize to be twice as big
            img.shift((t_i_up-0.5)*UP + 1.4*t_i_side*RIGHT)  # Move the image
        ...
        list2 = TexMobject(r"\text{increases with increasing}")
        list3 = TexMobject(r"\text{N, as relative side-lobe}")
        list4 = TexMobject(r"\text{area increases}")
        sentence=VGroup(list1,list2, list3, list4)
        sentence.arrange_submobjects(DOWN, buff=MED_LARGE_BUFF)
        sentence.shift((t_i_up-0.5)*UP + (t_i_side+ 0.5)*RIGHT)
        self.play(Write(sentence))
        ...
        for i in range(0,dtft_size):
            if (i < dtft_size_by_2):
                for j in range(0,dtft_size_by_2+i):
                    y[i] = y[i] + abs(coords[j][1]*rect_coords[dtft_size_by_2-i+j][1])
            else:
