...
class Scene3(GraphScene):
    ...
    def construct(self):
        ...
        list1 = TexMobject(r"\text{As, the window length}")
        list2 = TexMobject(r"\text{'N' increases}")      
        sentence=VGroup(list1,list2)
        sentence.arrange_submobjects(DOWN, buff=MED_LARGE_BUFF)
        sentence.shift(t_i_up*UP + t_i_side*LEFT)
        self.play(Write(sentence))

        for w_length in range(3,10,2):
            self.play(FadeOut(c_graph), ShowCreation(graph), run_time = 0.5)
            rect_coords = self.rect_dtft_2(w_length)
            rect_pts = self.get_points_from_coords(rect_coords)
            rect_window_ft = DiscreteGraphFromSetPoints(rect_pts, color = BLUE)
            img_name = './images/rect_window' + str(w_length) + '.jpg'
        ...
        sentence.shift(t_i_up*UP + t_i_side*LEFT)
        self.play(Write(sentence))
        list1 = TexMobject(r"\text{The Ripple Oscillations}")
        list2 = TexMobject(r"\text{increases with increasing N}")
        list3 = TexMobject(r"\text{due to decreasing relative}")
        list4 = TexMobject(r"\text{side-lobe area}")
        sentence=VGroup(list1,list2, list3, list4)
        sentence.arrange_submobjects(DOWN, buff=MED_LARGE_BUFF)
        sentence.shift((t_i_up-1)*UP + t_i_side*RIGHT)
        self.play(Write(sentence))
